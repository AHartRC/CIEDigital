...
using CIEDigitalLib.Search.Irony;
using Irony;
using Irony.Parsing;
...
namespace CIEDigitalLib.Extensions
{
    public static class SearchExtensions_Irony
    {
            ...
            params Expression<Func<T, string>>[] properties)
        {
            ...
            var searchGrammar = new SearchGrammar();
            var parser = new Parser(searchGrammar);
            ...
            if (parseTree.HasErrors() || parseTree.Root == null)
            {
                ...
                var errors = string.Join(
                    ...
                    parseTree.ParserMessages
                        .Where(m => m.Level == ErrorLevel.Error)
                        .Select(m => m.Message));
        ...
        private static Expression CreateChildExpression(ParseTreeNode node, MemberExpression[] properties,
            HashSet<string> terms)
        {
            switch (node.Term.Name)
            {
                ...
                default:
                    throw new InvalidOperationException("Grammar element '" + node.Term.Name +
                                                        "' is not handled correctly. Please investigate.");
            }
        }
    }
}