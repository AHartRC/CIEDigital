...
namespace CIEDigitalLib.Extensions
{
    ...
    public static class PagingExtensions
    {
            ...
            Func<T1, T2> convert)
        {
            ...
            return new PagedResult<T2>(query.SortAndPage(paging).AsEnumerable().Select(convert).ToArray(), count,
                paging);
        }
        ...
        public static IQueryable<T> SortAndPage<T>(this IQueryable<T> query, Paging paging)
        {
            ...
            {
                paging.SortColumn = typeof(T).GetProperties().First(p => p.PropertyType == typeof(string) || !p.PropertyType.GetInterfaces().Any(i => collections.Any(c => i == c))).Name;
            }
